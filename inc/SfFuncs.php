...
function queryLeadByEmail($email) {
    ...
    if (isset($auth->access_token) && isset($auth->instance_url)) {
        ...
        $query = "SELECT Id, Email, Phone, CreatedDate, Status FROM Lead WHERE Email = '" . $escapedEmail . "' AND IsConverted = false AND RecordTypeId = '01290000000sizUAAQ' ORDER BY CreatedDate DESC";
