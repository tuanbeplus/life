 ...
 */
function life_nav_menu($args = array(), $return_raw_items = false) {
  static $menu_id_slugs = array();

  $defaults = array('menu' => '', 'container' => 'nav', 'container_class' => '', 'container_id' => '',
            'menu_class' => 'menu', 'menu_id' => '',
            'echo' => true, 'fallback_cb' => 'wp_page_menu', 'before' => '', 'after' => '', 'link_before' => '', 'link_after' => '',
            'items_wrap' => '<ul id="%1$s" class="%2$s">'."\n".'%3$s</ul>',
            'depth' => 0, 'walker' => '', 'theme_location' => '', 'indent' => 0
           );
  ...
  $args->walker = new life_Walker_Nav_Menu();
  $items .= life_walk_nav_menu_tree($sorted_menu_items, $args->depth, $args);
   ...
   * @param array  $args     Array of wp_nav_menu() arguments.
   */
  $nav_menu = apply_filters( 'life_nav_menu', $nav_menu, $args );
  
  $indented = '';
  foreach ( explode("\n", $nav_menu) as $line ) {
    $indented .= str_repeat("\t", $args->indent).$line."\n";
  }
 ...
 */
function life_walk_nav_menu_tree( $items, $depth, $r ) {
  $walker = ( empty($r->walker) ) ? new life_Walker_Nav_Menu : $r->walker;
...
function life_nav_items($menu_id, $default = null) {
  static $_nav_items = [];
  static $locations = null;
  if (!isset($_nav_items[$menu_id])) {
    if ($locations === null) {
      $locations = get_nav_menu_locations();
    }
    $_nav_items[$menu_id] = null;
    if (isset($locations[$menu_id])) {
      $raw_items = wp_get_nav_menu_items(get_term($locations[$menu_id], 'nav_menu'));
  
      $refs = [];
      $_nav_items[$menu_id] = [];
  
      foreach (array_map(fn ($raw_item) => (object)[
        'id' => $raw_item->ID,
        'menu_item_parent' => $raw_item->menu_item_parent,
        'target' => $raw_item->target,
        'childItems' => [],
        'classes' => $raw_item->classes,
        'title' => apply_filters('the_brand', $raw_item->title),
        'url' => $raw_item->url,
      ], $raw_items) as $item) {
        $refs[$item->id] = $item;
        if ($item->menu_item_parent) {
          $refs[$item->menu_item_parent]->childItems[] = $item;
        } else {
          $_nav_items[$menu_id][] = $item;
        }
      }
    }
  }
  return $_nav_items[$menu_id] ?? $default;
}